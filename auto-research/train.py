...
Experiment: Three-stream ResNet18 shared encoder (far + near + card)
Uses ALL 3 images through a shared pretrained ResNet18 encoder.
Adds face-card difference features to help detect deepfake inconsistencies.
"""
...
class TripleStreamResNet(nn.Module):
    """Three-stream ResNet18: shared encoder for far, near, card.
    Concatenates features + element-wise differences (far-card, near-card)
    to explicitly model face-card consistency."""
    def __init__(self, num_classes=2):
        super().__init__()
        base = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
        self.encoder = nn.Sequential(*list(base.children())[:-1])
        feat_dim = 512
        # 3 streams (1536) + 2 difference vectors (1024) = 2560
        combined_dim = feat_dim * 3 + feat_dim * 2
...
def train():
    ...
    print(f"Model: Triple-stream ResNet18 shared encoder (far+near+card) + diff features, params: {num_params:,}")
    ...
    approach = "triple_stream_resnet18_shared_encoder_far_near_card_diff_features_TTA"
    result_block = (
        f"\n---\n"
