...
cross-stream consistency features + cosine LR + TTA + focal loss.

KEY INSIGHT: All previous best experiments only used 2 images (far + near).
The card image contains the REFERENCE face on the ID document. Human annotators
rely heavily on comparing face vs card to detect deepfake/injection attacks.
Adding the card stream + explicit cross-stream difference features should
capture face-card inconsistency that is the hallmark of ~70% of attacks.
"""
...
class LivenessDatasetTriple(Dataset):
    """Dataset that loads all 3 images: far, near, and card."""
...
class TripleStreamResNet(nn.Module):
    """
    Triple-stream architecture with shared ResNet18 encoder.
    Processes far, near, and card images through the same encoder.
    Fuses features via concatenation + explicit cross-stream difference vectors.
    
    The difference features (far-card, near-card, far-near) capture cross-modal
    consistency, which is key for detecting deepfake injection attacks where the
    submitted face doesn't match the ID card face.
    """
    def __init__(self, num_classes=2):
        ...
        # 3 streams (1536) + 3 difference vectors (1536) = 3072 total
        fused_dim = feat_dim * 6
...
def train():
    ...
    print(f"Model: Triple-stream ResNet18 shared encoder + cross-stream diffs, params: {num_params:,}")
    ...
    approach = "triple_stream_resnet18_shared_far_near_card_cross_diffs_focal_TTA"
    result_block = (
        f"\n---\n"
