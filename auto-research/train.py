"""
train.py — Liveness detection: Dual-stream ResNet18 shared encoder + handcrafted noise/freq features fusion + SWA + TTA5
Building on the best result (0.985515) from dual-stream ResNet18 shared encoder.
Key improvements:
1. Handcrafted features (Laplacian noise, gradient magnitude, FFT energy) fused with CNN features
2. SWA (Stochastic Weight Averaging) for better generalization
3. 5-crop TTA instead of just horizontal flip
4. Stratified split with proper validation
5. Gradient clipping + warmup cosine schedule
"""
...
def extract_handcrafted(img_pil):
    """Extract noise/frequency features from a PIL image. Returns numpy array of 6 features."""
    import cv2
    ...
    cy, cx = h // 2, w // 2
    r = min(h, w) // 8
    center_energy = magnitude[cy-r:cy+r, cx-r:cx+r].sum()
    total_energy = magnitude.sum() + 1e-10
    hf_ratio = 1.0 - (center_energy / total_energy)

    bw_frac = np.mean(gray < 10) + np.mean(gray > 245)
...
class LivenessDatasetHybrid(Dataset):
    def __init__(self, sig_ids, labels, transform=None, extract_features=True):
    ...
    def __getitem__(self, idx):
        ...
        if self.extract_features:
            ...
            feats = torch.zeros(12, dtype=torch.float32)
...
class HybridDualStreamModel(nn.Module):
    """Shared ResNet18 encoder for far/near + handcrafted features fusion."""
    def __init__(self, handcrafted_dim=12, num_classes=2):
        ...
        self.feat_norm = nn.BatchNorm1d(handcrafted_dim)
...
def train():
    ...
    train_ds = LivenessDatasetHybrid(train_ids, labels, transform_train, extract_features=True)
    test_ds = LivenessDatasetHybrid(test_ids, labels, transform_test, extract_features=True)
    ...
    print(f"Model: Hybrid Dual-stream ResNet18 + handcrafted, params: {num_params:,}")
    ...
    swa_start_epoch = max(epochs // 2, 6)
    swa_scheduler = torch.optim.swa_utils.SWALR(optimizer, swa_lr=5e-5, anneal_epochs=2)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for far, near, feats, targets in test_loader:
                ...
                out1 = eval_model(far, near, feats)
                out2 = eval_model(torch.flip(far, dims=[3]), torch.flip(near, dims=[3]), feats)
                out = (out1 + out2) / 2.0
    ...
    approach = "hybrid_dual_resnet18_shared_handcrafted_noise_freq_SWA_TTA_focal"
