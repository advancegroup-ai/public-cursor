...
with cross-stream difference features for face-card consistency detection.

Key idea: The card image has the reference face on the ID document.
Deepfakes (70% of negatives) submit real card but synthetic face.
By comparing card features vs face features, we can detect mismatches.

Architecture:
  - Shared pretrained ResNet18 encoder for all 3 images
  - Feature concatenation: [far, near, card, far-card, near-card] = 5*512 = 2560
  - Bottleneck MLP classifier
  - Cosine LR + TTA (horizontal flip)
"""
...
class TripleStreamResNet(nn.Module):
    """Triple-stream with shared ResNet18 + cross-stream difference features."""
    def __init__(self, num_classes=2):
        super().__init__()
        base = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
        self.encoder = nn.Sequential(*list(base.children())[:-1])
        feat_dim = 512
        # [far, near, card, far-card, near-card] = 5 * 512 = 2560
    ...
    def forward(self, far, near, card):
        ...
        combined = torch.cat([f_far, f_near, f_card, diff_far_card, diff_near_card], dim=1)
...
def train():
    ...
    print(f"Model: Triple-stream ResNet18 shared encoder + diff features, params: {num_params:,}")
    ...
    approach = "triple_stream_resnet18_shared_far_near_card_diff_features_TTA"
    result_block = (
        f"\n---\n"
